...
tidy_qda <- function(phylum.matrix, metadata.tb, grouping.var){
  #' Tidy LDA
  #'
  #' Perform MASS::qda on a dataset, specifying the grouping
  #' as a column of your metadata.tb
  #'
  #' @param phylum.matrix A matrix with taxa as rows and samples as columns
  #' @param metadata.tb A tibble with samples as rows and metadata as columns
  ...
  # Perform lda
  LDA <- 
    suppressWarnings(MASS::qda(t(phylum.matrix), grouping=.grouper))
